...
const updateItem = async ({ id, name, fields, status }) => {
  ...
  const existReportForm = await reportFormRepo.findItemById({
    id,
  });
  ...
  if (name && name !== existReportForm.name) {
    ...
    const existReportFormByName = await reportFormRepo.findItemById({
      qry,
    });
  ...
  const reportForm = await reportFormRepo.updateItemById({
    id,
